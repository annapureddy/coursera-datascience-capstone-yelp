...
library(dplyr)
users_to_consider <- reviews_to_consider %>% group_by(user_id, business_id) %>% summarize(count1 = n()) %>% group_by(user_id) %>% summarize(count = n()) %>% arrange(desc(count)) %>% filter(count >= 3) %>% mutate(total_count = cumsum(count)) %>% filter(total_count < 100000) %>% select(user_id)
...
algorithms <- list("Random" = list(name = "RANDOM", param = NULL),
                   ...
                   "Item-based CF" = list(name = "RANDOM", param = NULL),
                   "PCA" = list(name = "PCA", param = NULL),
                   "SVD" = list(name = "SVD", param = NULL))

# Run the above algorithms with cross-validation
r <- as(reviews_to_consider, "realRatingMatrix")
scheme <- evaluationScheme(r, method = "cross", given = 3)
results <- evaluate(scheme, algorithms, n = c(5, 10, 15))
